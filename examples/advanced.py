...
class DatabaseManager:
    ...
    def execute_query(self, sql: str):
        ...
        query_log.trace("Query performance", field("duration_ms", duration_ms), field("rows_affected", 42))
...
def handle_http_request(req: HttpRequest):
    ...
    try:
        ...
        if req.path == "/api/users":
            ...
            request_log.debug("Response details", field("status_code", 200), field("response_size", 1024))
...
def worker_thread(thread_id: int):
    ...
    for i in range(5):
        if thread_id % 4 == 0:
            ...
            log.verbose("Processing item with verbose details", 
                       field("item", i), field("memory_mb", 128 + i * 10))
        elif thread_id % 4 == 2:
            log.debug("Processing item with debug info", 
                     field("item", i), field("cpu_percent", 15.5 + i * 2.1))
        else:
            log.trace("Processing item with trace details", 
                     field("item", i), field("timestamp", int(time.time() * 1000)))
