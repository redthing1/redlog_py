...
from redlog import (
    get_logger, set_level, set_theme, get_theme, Level, field, level_name,
    Theme, Color, themes, DefaultFormatter, TimestampedFormatter, JSONFormatter,
    StringSink, ConsoleSink, Logger
)
...
class ServerStats:
    ...
    def __str__(self):
        return f"ServerStats{{conn={self.connections}, cpu={self.cpu_usage}%, mem={self.memory_mb}MB}}"
...
def demonstrate_environment_variables():
    ...
    print(f"- REDLOG_NO_COLOR: {'set (colors disabled)' if os.getenv('REDLOG_NO_COLOR') else 'not set'}")
    print(f"- FORCE_COLOR: {'set (colors forced)' if os.getenv('FORCE_COLOR') else 'not set'}")
    print(f"- REDLOG_FORCE_COLOR: {'set (colors forced)' if os.getenv('REDLOG_FORCE_COLOR') else 'not set'}")
...
def demonstrate_printf_formatting():
    ...
    log.info_f(
        ...
        "web-server-01", 8080, 85.7, 256, 0o755
    )
